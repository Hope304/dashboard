...
class ShapefileController extends Controller
{
    ...
    public function postUpload(Request $request)
    {
        ...
        if (count($arr_check) == 0) {
            ...
            try {
                ...
                return redirect("admin/shp/create")->with('err', 'Geometry không khớp kiểu dữ liệu, vui lòng chuyển geometry về định dạng MultiPolygon');
            }
    ...
    public function getSuDung($id)
    {
        ...
        try {
            ...
            return redirect("admin/shp")->with('err', 'Geometry không khớp kiểu dữ liệu, vui lòng chuyển geometry về định dạng MultiPolygon');
        }
    ...
    public function getBando($id)
    {
        ...
        if (file_exists($path)) {
            ...
            try {
                ...
                return redirect("admin/shp")->with('fail', 'Geometry không khớp kiểu dữ liệu, vui lòng chuyển geometry về định dạng MultiPolygon');
            }
        } else {
    ...
    public function getApprove($id)
    {
        ...
        try {
            ...
            $subject = 'Kết quả kiểm tra và phê duyệt SHP file ' . $data->ten . ' (upload bởi ' . User::findOrFail($data->id_user)->name . ' ' . $data->ngaytao . ')';
            Mail::send('sendMail.shp_admin', ['type' => 2], function ($mess) use ($subject, $admin_email) {
                $mess->to($admin_email);
                $mess->from('xmg-dichvu@ifee.edu.vn', 'Hệ thống giám sát rừng tỉnh Hà Nam');
                $mess->subject($subject);
            });
        ...
        return redirect("admin/shp")->with('success', 'Phê duyệt thành công');
    }

    public function getRefuse($id, Request $request)
    {
        $data = UpShapFile::findOrFail($id);
        try {
            $admin_email = trim(User::where('role', 'admin')->first()->email);
            ...
            $subject = 'Kết quả kiểm tra và phê duyệt SHP file ' . $data->ten . ' (upload bởi ' . User::findOrFail($data->id_user)->name . ' ' . $data->ngaytao . ')';
            Mail::send('sendMail.shp_admin', ['type' => 3, 'lydo' => $request->lydo], function ($mess) use ($subject, $admin_email) {
                $mess->to($admin_email);
                $mess->from('xmg-dichvu@ifee.edu.vn', 'Hệ thống giám sát rừng tỉnh Hà Nam');
                $mess->subject($subject);
            });
        ...
        return redirect("admin/shp")->with('success', 'Từ chối thành công');
    }
}
